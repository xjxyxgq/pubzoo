...
def analyze_host_usage(data_file_path, ip_file_path, show_all=False):
    ...
    # 确保数值型列的数据类型正确
    numeric_columns = ['最大CPU', '最大内存', '最大磁盘']
    for col in numeric_columns:
        filtered_df[col] = pd.to_numeric(filtered_df[col], errors='coerce')
    
    # 处理重复的IP记录，合并时对最大CPU、最大内存、最大磁盘取较大值
    original_count = len(filtered_df)
    if original_count > len(found_ips):
        print(f"发现重复的IP记录，原始记录数: {original_count}，唯一IP数: {len(found_ips)}")
    
    # 总是进行去重处理，按IP分组并对各个指标取最大值
    filtered_df = filtered_df.groupby('IP地址').agg({
        '最大CPU': 'max',
        '最大内存': 'max', 
        '最大磁盘': 'max'
    }).reset_index()
    ...
    # 检查是否有NaN值并报告
    nan_count = filtered_df[numeric_columns].isna().sum().sum()
    if nan_count > 0:
        print(f"Warning: 发现 {nan_count} 个无效数值，将被忽略")
        # 填充NaN为0或删除包含NaN的行，这里选择填充为0
        filtered_df[numeric_columns] = filtered_df[numeric_columns].fillna(0)
    
    final_count = len(filtered_df)
    if original_count != final_count:
        print(f"合并重复记录后，最终记录数: {final_count}")
    
    print(f"在监控数据中找到 {final_count} 个IP，共 {len(target_ips)} 个目标IP")
